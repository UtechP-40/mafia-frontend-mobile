...
export const socketMiddleware: Middleware<{}, RootState> = (store) => (next) => (action) => {
  ...
  if (action.type === 'auth/loginSuccess' || action.type === 'auth/loginUser/fulfilled') {
  ...
  const socketEvent = SOCKET_ACTIONS[action.type as keyof typeof SOCKET_ACTIONS];
...
function connectSocket(store: any, token: string) {
  ...
  socketService.on('phase-changed', (data) => {
    ...
    const message = phaseMessages[data.phase as keyof typeof phaseMessages] || `Phase changed to ${data.phase}`;
    store.dispatch(addNotification({
      message,
      type: 'info',
      duration: 4000,
